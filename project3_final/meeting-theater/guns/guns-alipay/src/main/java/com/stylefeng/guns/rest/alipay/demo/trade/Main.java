package com.stylefeng.guns.rest.alipay.demo.trade;
...
import com.stylefeng.guns.rest.alipay.demo.trade.config.Configs;
import com.stylefeng.guns.rest.alipay.demo.trade.model.ExtendParams;
import com.stylefeng.guns.rest.alipay.demo.trade.model.GoodsDetail;
import com.stylefeng.guns.rest.alipay.demo.trade.model.builder.AlipayHeartbeatSynRequestBuilder;
import com.stylefeng.guns.rest.alipay.demo.trade.model.builder.AlipayTradePayRequestBuilder;
import com.stylefeng.guns.rest.alipay.demo.trade.model.builder.AlipayTradePrecreateRequestBuilder;
import com.stylefeng.guns.rest.alipay.demo.trade.model.builder.AlipayTradeRefundRequestBuilder;
import com.stylefeng.guns.rest.alipay.demo.trade.model.hb.*;
import com.stylefeng.guns.rest.alipay.demo.trade.model.result.AlipayF2FPayResult;
import com.stylefeng.guns.rest.alipay.demo.trade.model.result.AlipayF2FPrecreateResult;
import com.stylefeng.guns.rest.alipay.demo.trade.model.result.AlipayF2FRefundResult;
import com.stylefeng.guns.rest.alipay.demo.trade.service.AlipayMonitorService;
import com.stylefeng.guns.rest.alipay.demo.trade.service.AlipayTradeService;
import com.stylefeng.guns.rest.alipay.demo.trade.service.impl.AlipayMonitorServiceImpl;
import com.stylefeng.guns.rest.alipay.demo.trade.service.impl.AlipayTradeServiceImpl;
import com.stylefeng.guns.rest.alipay.demo.trade.service.impl.AlipayTradeWithHBServiceImpl;
import com.stylefeng.guns.rest.alipay.demo.trade.utils.Utils;
import com.stylefeng.guns.rest.alipay.demo.trade.utils.ZxingUtils;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 测试当面付2.0生成支付二维码
        String s = main.getPayQRCode("2", "大地影院(顺义店)", "60", "1");
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String sellerId = "2088102180460082";
    ...
    public String queryPayResult(String orderId) {
        AlipayClient alipayClient = new DefaultAlipayClient("https://openapi.alipay.com/gateway.do", "2016102100729100", "your private_key", "json", "GBK", "alipay_public_key", "RSA2");
    ...
    public String getPayQRCode(String orderId, String cinemaName, String price, String cinemaId) {
        ...
        // 我们这里建议大家传订单ID  (发号器 的设计)
        String outTradeNo = (orderId);

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "Mtime网站" + cinemaName + "门店当面付扫码消费";
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        //GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", "我不是药神电影票", 3000, 1);
        // 创建好一个商品后添加至商品明细列表
        //goodsDetailList.add(goods1);

        // 继续创建并添加第一条商品信息，用户购买的产品为“黑人牙刷”，单价为5.00元，购买了两件
        //GoodsDetail goods2 = GoodsDetail.newInstance("goods_id002", "我不是药神电影票", 3000, 1);
        //goodsDetailList.add(goods2);
        ...
        String s = "alipay/" + UUID.randomUUID().toString() + ".png";
        switch (result.getTradeStatus()) {
            case SUCCESS:
                log.info("支付宝预下单成功: )");
                ...
                String filePath = String.format("D:\\File\\" + s, response.getOutTradeNo());
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath); // GUI
                break;
